 ...
 *
 * <p>**不区分环境**：任何 profile 下，配置了 {@code ALIYUN_SMS_ACCESS_KEY_ID/SECRET} 即真实发送
 * （ProdCodeSender → AliyunSmsSender 阿里云短信），未配置则降级日志旁路（DevCodeSender）——
 * 开发/测试环境配 key 后同样可真实发短信（H5 验证码登录），不配也不会启动失败。
 *
 * <p>读取环境变量：
 *
 * <ul>
...
public class AliyunSmsConfig {
  ...
  public CodeSender codeSender(
    ...
    if (accessKeyId.isBlank() || accessKeySecret.isBlank()) {
      log.warn(
          "阿里云短信未配置（ALIYUN_SMS_ACCESS_KEY_ID/SECRET 缺失），验证码发送走日志旁路（DevCodeSender）");
    ...
    try {
      ...
      log.info(
          "阿里云短信已配置（真实发送），emailSender 装配={}",
          emailSender != null ? "是" : "否（邮件通道降级）");
